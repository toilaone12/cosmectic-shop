...
<script>
    $(function() {
        ...
        notyf.error({
            message: '{{session("errorOrder")}}'
        })
        @elseif(session('successOrder'))
        notyf.success({
            message: '{{session("successOrder")}}'
        });
        @endif
        //mo modal
        ...
        //them nhanh gio hang (ngoai trang chu)
        $(document).on('click', '.add-this-product', function(e) {
            e.preventDefault();
            let id = $(this).attr('data-id');
        ...
        $('.qty-click-cart').on('click', function(e) {
            ...
            if (type == 'up') {
                if (qty >= max) {
                    qtyUpdate = max;
                    notyf.error({
                        message: 'Đã quá số lượng sản phẩm trong kho'
                    });
                    return false;
                } else {
                    qtyUpdate = qty + 1;
        ...
        $(document).on('click', '.apply-address', function(e) {
            ...
            if (keyword && lat && lng) {
                ...
                notyf.error({
                    message: 'Bạn chưa chọn địa chỉ đặt hàng vui lòng đặt lại'
                });
            }
        })
        //dat hang
        $('#order').on('submit', function(e) {
            ...
            if (!checkPayment) { //check chua chon phuong thuc giao hang
                notyf.error({
                    message: 'Bạn chưa chọn phương thức giao hàng'
                });
                errors = {
                    'payment': 1
                };
            ...
            if (!terms) { //chua co dieu khoan
                notyf.error({
                    message: 'Bạn chưa đồng ý với điều khoản mua hàng'
                });
                errors = {
                    'terms': 1
                };
            ...
            if (paymentOnline) { //check vi dien tu
                if (!chooseCard) {
                    notyf.error({
                        message: 'Bạn chưa chọn ví điện tử'
                    });
                    errors = {
                        'card': 1
                    };
